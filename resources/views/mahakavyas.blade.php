...
<body>
    <ul class="nav nav-tabs" id="libraryTab" role="tablist" style="margin-top:-5%;">
        <li class="nav-item">
            <a class="nav-link {{ Route::currentRouteName() == 'vedas' ? 'active' : '' }}" id="vedas-tab" href="{{ route('vedas') }}" role="tab" aria-controls="vedas" aria-selected="{{ Route::currentRouteName() == 'vedas' ? 'true' : 'false' }}" style="color:#f39c12">Vedas</a>
        </li>
        <li class="nav-item">
            <a class="nav-link {{ Route::currentRouteName() == 'puranas' ? 'active' : '' }}" id="puranas-tab" href="{{ route('puranas') }}" role="tab" aria-controls="puranas" aria-selected="{{ Route::currentRouteName() == 'puranas' ? 'true' : 'false' }}" style="color:#f39c12">Puranas</a>
        </li>
        <li class="nav-item">
            <a class="nav-link {{ Route::currentRouteName() == 'mahakavyas' ? 'active' : '' }}" id="mahakavyas-tab" href="{{ route('mahakavyas') }}" role="tab" aria-controls="mahakavyas" aria-selected="{{ Route::currentRouteName() == 'mahakavyas' ? 'true' : 'false' }}" style="color:#f39c12">Mahakavyas</a>
        </li>
    </ul>
    <br><br><br><br>
...
function addToWishlist(event, bookId) {
    event.stopPropagation(); // Prevent the book click event

    fetch(`/add-wishlist/${bookId}`, {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            alert(data.success);
            window.location.href = data.redirect; // Redirect to wishlist
        } else {
            alert(data.error);
        }
    })
